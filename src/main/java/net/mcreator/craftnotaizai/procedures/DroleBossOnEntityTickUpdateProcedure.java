...
public class DroleBossOnEntityTickUpdateProcedure {
	public static void execute(LevelAccessor world, double y, Entity entity) {
		...
		if (entity.getPersistentData().getDouble("skill_cooldown") == 0 && !((entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null) == null)) {
			...
			if (distance <= 6) {
				...
				if (ran == 3) {
					for (int index1 = 0; index1 < (int) (15 * 1); index1++) {
						if (world instanceof ServerLevel _level) {
							Entity entityToSpawn = CraftNoTaizaiModEntities.GIGA_CRUSH.get().spawn(_level, BlockPos.containing(entity.getX() + entity.getLookAngle().x + Mth.nextInt(RandomSource.create(), -5, 5),
									y + Mth.nextInt(RandomSource.create(), -5, -3), entity.getZ() + entity.getLookAngle().z + Mth.nextInt(RandomSource.create(), -5, 5)), MobSpawnType.MOB_SUMMONED);
							if (entityToSpawn != null) {
							}
						}
					}
			...
			entity.getPersistentData().putDouble("skill_cooldown", (Mth.nextInt(RandomSource.create(), 150, 400)));
		}
	}
}
