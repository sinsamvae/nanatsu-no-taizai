...
public class BlockdodgeteleportProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, DamageSource damagesource, Entity entity) {
		...
		if (entity.getType().is(TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation("craft_no_taizai:sds_entities")))) {
			...
			if (damagesource.is(DamageTypes.PLAYER_ATTACK) || damagesource.is(ResourceKey.create(Registries.DAMAGE_TYPE, new ResourceLocation("craft_no_taizai:mana_dmg"))) || damagesource.is(DamageTypes.ARROW)) {
				if (Random == 5) {
