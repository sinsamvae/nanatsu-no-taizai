		...
		if (entity.getPersistentData().getDouble("skill_cooldown") == 0 && !((entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null) == null)) {
			...
			if (distance <= 6) {
				if (entity instanceof DuskBison1Entity) {
					((DuskBison1Entity) entity).setAnimation("attack");
				}
				{
					...
					for (Entity entityiterator : _entfound) {
						if (!(entityiterator == entity || entityiterator instanceof ItemEntity || entityiterator instanceof ExperienceOrb
							...
							CraftNoTaizaiMod.queueServerWork(24, () -> {
								...
								entityiterator.setDeltaMovement(new Vec3(((entityiterator.getDeltaMovement().x() + entityiterator.getLookAngle().x) * 0), ((entityiterator.getDeltaMovement().y() + entityiterator.getLookAngle().y) * (-6)),
			...
			entity.getPersistentData().putDouble("skill_cooldown", 60);
		}
	}
}
