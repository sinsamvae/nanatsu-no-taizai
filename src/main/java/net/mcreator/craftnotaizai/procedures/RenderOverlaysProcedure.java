...
public class RenderOverlaysProcedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			RenderOverlaysProcedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				RenderOverlaysProcedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	private static void execute(@Nullable Event event) {
		if (target(2)) {
			if (Minecraft.getInstance().player != null) {
				...
				renderTexts(BPProcedure.execute(world, entity), 25, 8, 0, 0, (float) 0.8, 255 << 24 | 255 << 16 | 255 << 8 | 255, 0);
				renderTexts(
