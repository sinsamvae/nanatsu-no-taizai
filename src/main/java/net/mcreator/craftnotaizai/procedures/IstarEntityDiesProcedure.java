...
public class IstarEntityDiesProcedure {
	@SubscribeEvent
		...
		if (entity.getType().is(TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation("craft_no_taizai:cave_of_training")))) {
			{
				...
				for (Entity entityiterator : _entfound) {
					if (entityiterator instanceof Player) {
						{
							double _setval = (entityiterator.getCapability(CraftNoTaizaiModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CraftNoTaizaiModVariables.PlayerVariables())).remainingEnemies - 1;
