...
public class MelasculaBossOnEntityTickUpdateProcedure {
	public static void execute(LevelAccessor world, double y, Entity entity) {
		...
		if (entity.getPersistentData().getDouble("skill_cooldown") == 0 && !((entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null) == null)) {
			...
			if (distance <= 6) {
				...
				if (ran == 2) {
					if (!(!world.getEntitiesOfClass(RedDemonGeckolibEntity.class, AABB.ofSize(new Vec3(x, y, z), 25, 25, 25), e -> true).isEmpty())) {
						y_offset = 3;
