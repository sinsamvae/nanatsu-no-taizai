...
public class GalandBossOnEntityTickUpdateProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		double distance = 0;
		double ran = 0;
		if ((entity instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) < ((entity instanceof LivingEntity _livEnt ? _livEnt.getMaxHealth() : -1) / 100) * 25) {
			if (!entity.level().isClientSide())
				entity.discard();
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = CraftNoTaizaiModEntities.GALAND_OVER_BOSS.get().spawn(_level, BlockPos.containing(x, y, z), MobSpawnType.MOB_SUMMONED);
		...
		if (entity.getPersistentData().getDouble("skill_cooldown") == 0 && !((entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null) == null)) {
			...
			if (distance <= 6) {
				if (ran == 1) {
					if (entity instanceof GalandBossEntity) {
						((GalandBossEntity) entity).setAnimation("attack_2");
					}
				...
				if (ran == 2) {
					if (entity instanceof GalandBossEntity) {
						((GalandBossEntity) entity).setAnimation("attack_3");
					}
					CraftNoTaizaiMod.queueServerWork(8, () -> {
						{
