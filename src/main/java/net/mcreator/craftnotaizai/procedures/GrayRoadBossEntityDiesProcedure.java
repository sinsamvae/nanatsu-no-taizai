...
public class GrayRoadBossEntityDiesProcedure {
		...
		if (!(entity instanceof Player)) {
			{
				...
				for (Entity entityiterator : _entfound) {
					...
					if (entityiterator instanceof Player && !(entityiterator instanceof ServerPlayer _plr9 && _plr9.level() instanceof ServerLevel
							&& _plr9.getAdvancements().getOrStartProgress(_plr9.server.getAdvancements().getAdvancement(new ResourceLocation("craft_no_taizai:defeat_gray_road"))).isDone())) {
						if (entityiterator instanceof ServerPlayer _player) {
							Advancement _adv = _player.server.getAdvancements().getAdvancement(new ResourceLocation("craft_no_taizai:defeat_gray_road"));
