...
public class DerieriBossEntityDiesProcedure {
		...
		double drop = 0;
		{
			...
			for (Entity entityiterator : _entfound) {
				...
				if (entityiterator instanceof Player && !(entityiterator instanceof ServerPlayer _plr8 && _plr8.level() instanceof ServerLevel
						&& _plr8.getAdvancements().getOrStartProgress(_plr8.server.getAdvancements().getAdvancement(new ResourceLocation("craft_no_taizai:defeat_derieri"))).isDone())) {
					if (entityiterator instanceof ServerPlayer _player) {
						Advancement _adv = _player.server.getAdvancements().getAdvancement(new ResourceLocation("craft_no_taizai:defeat_derieri"));
